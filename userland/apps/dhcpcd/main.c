...
static int parse_ipv4_quad(const char *s, unsigned char out[4])
{
...
int main(int argc, char **argv)
{
    ...
    /* ── Check /etc/network.conf for static IP configuration ─────────── */
    int cfg_fd = open("/etc/network.conf", O_RDONLY, 0);
    if (cfg_fd >= 0) {
        char cbuf[512];
        ssize_t cn = read(cfg_fd, cbuf, sizeof(cbuf) - 1);
        close(cfg_fd);
        if (cn > 0) {
            cbuf[cn] = '\0';
            char *dhcp_opt = strstr(cbuf, "dhcp=");
            int dhcp_val = 1;
            if (dhcp_opt) dhcp_val = atoi(dhcp_opt + 5);
            if (dhcp_val == 0) {
                char ip_s[32] = "10.0.2.15";
                char nm_s[32] = "255.255.255.0";
                char gw_s[32] = "10.0.2.2";
                char dns_s[32] = "10.0.2.3";
                char *p = strstr(cbuf, "ip=");
                if (p) sscanf(p + 3, "%31s", ip_s);
                p = strstr(cbuf, "netmask=");
                if (p) sscanf(p + 8, "%31s", nm_s);
                p = strstr(cbuf, "gateway=");
                if (p) sscanf(p + 8, "%31s", gw_s);
                p = strstr(cbuf, "dns=");
                if (p) sscanf(p + 4, "%31s", dns_s);

                unsigned char ip[4], nm[4], gw[4], dns[4];
                parse_ipv4_quad(ip_s, ip);
                parse_ipv4_quad(nm_s, nm);
                parse_ipv4_quad(gw_s, gw);
                parse_ipv4_quad(dns_s, dns);

                int net_fd = open("/dev/net0", O_RDWR, 0);
                if (net_fd >= 0) {
                    ioctl(net_fd, 0x8916 /* SIOCSIFADDR */, (unsigned long)ip);
                    ioctl(net_fd, 0x891c /* SIOCSIFNETMASK */, (unsigned long)nm);
                    ioctl(net_fd, 0x891e /* SIOCSIFGW */, (unsigned long)gw);
                    ioctl(net_fd, 0x8921 /* SIOCSIFDNS */, (unsigned long)dns);
                    int flags = 0x4163;
                    ioctl(net_fd, 0x8914 /* SIOCSIFFLAGS */, (unsigned long)&flags);
                    close(net_fd);
                }
                ...
                if (resolv_fd >= 0) {
                    ...
                    int rlen = snprintf(resolv_buf, sizeof(resolv_buf),
                        "# Generated by static network configuration for AzamiOS\n"
                        "nameserver %s\n", dns_s);
                    write(resolv_fd, resolv_buf, (size_t)rlen);
                    close(resolv_fd);
                }

                printf("dhcpcd: static configuration active (/etc/network.conf: dhcp=0)\n");
                printf("dhcpcd: applied static IP to net0:\n");
                printf("        IP:         %s\n", ip_s);
                printf("        Netmask:    %s\n", nm_s);
                printf("        Gateway:    %s\n", gw_s);
                printf("        DNS:        %s\n", dns_s);
    ...
    if (net_fd >= 0) {
        ...
        for (int wait = 0; wait < 10; wait++) {
            if (ioctl(net_fd, SIOCGIFDHCP, (unsigned long)&lease) == 0 && lease.state == 3 /* BOUND */) {
                /* Write /etc/resolv.conf, but only if the DHCP server
                 * actually handed out a DNS option -- writing a bogus
                 * 0.0.0.0 nameserver would be worse than leaving whatever
                 * config already existed untouched. */
                if (lease.dns[0] || lease.dns[1] || lease.dns[2] || lease.dns[3]) {
                    int resolv_fd = open("/etc/resolv.conf", O_WRONLY | O_CREAT | O_TRUNC, 0644);
                    if (resolv_fd >= 0) {
                        char resolv_buf[256];
                        int rlen = snprintf(resolv_buf, sizeof(resolv_buf),
                ...
                printf("dhcpcd: lease BOUND successfully on net0:\n");
    ...
    /* Fallback: Direct socket-based DHCP implementation */
    ...
    *opt++ = DHCP_OPT_END;
    ...
    /* 2. Receive DHCPOFFER with timeout */
    dhcp_packet_t resp;
    ...
    int rcv_timeout = 2000; /* 2 seconds */
    ...
    if (n > 0 && ntohl(resp.magic_cookie) == DHCP_MAGIC_COOKIE) {
        ...
        *opt++ = DHCP_OPT_END;
        ...
        /* 4. Receive DHCPACK */
        recvfrom(sockfd, &resp, sizeof(resp), 0, NULL, NULL);
    }
    ...
    if (offered_ip[0] != 0 || offered_ip[1] != 0 || offered_ip[2] != 0 || offered_ip[3] != 0) {
        ...
        /* 6. Write /etc/resolv.conf, only when the offer actually included
         * a DNS server. */
