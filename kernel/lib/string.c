...
void *memcpy(void *dest, const void *src, size_t n)
{
    ...
    if ((((uintptr_t)d | (uintptr_t)s) & 7) == 0) {
        ...
        d = (unsigned char *)dst64;
        s = (const unsigned char *)src64;
    }
...
static void buf_puts(char *buf, size_t size, size_t *pos, const char *s, int prec)
{
    while (*s && (prec < 0 || prec-- > 0)) {
        buf_putc(buf, size, pos, *s++);
    }
}

static void buf_u64(char *buf, size_t size, size_t *pos, u64 v, u32 base, bool upper, u32 width, char pad)
{
...
static void buf_s64(char *buf, size_t size, size_t *pos, s64 v, u32 width, char pad)
{
    bool neg = false;
    u64 uv;
    if (v < 0) {
...
int vsnprintf(char *buf, size_t size, const char *fmt, __builtin_va_list ap)
{
    ...
    for (const char *p = fmt; *p; p++) {
        ...
        case 'd': case 'i': {
            ...
            buf_s64(buf, size, &pos, v, width, pad);
            break;
        }
        case 'u': {
            u64 v = is_llong ? __builtin_va_arg(ap, u64)
                             : (is_long ? __builtin_va_arg(ap, unsigned long) : __builtin_va_arg(ap, unsigned int));
            buf_u64(buf, size, &pos, v, 10, false, width, pad);
            break;
        }
        case 'x': {
            u64 v = is_llong ? __builtin_va_arg(ap, u64)
                             : (is_long ? __builtin_va_arg(ap, unsigned long) : __builtin_va_arg(ap, unsigned int));
            buf_u64(buf, size, &pos, v, 16, false, width, pad);
            break;
        }
        case 'X': {
            u64 v = is_llong ? __builtin_va_arg(ap, u64)
                             : (is_long ? __builtin_va_arg(ap, unsigned long) : __builtin_va_arg(ap, unsigned int));
            buf_u64(buf, size, &pos, v, 16, true, width, pad);
            break;
        }
        case 'p': {
            uintptr_t v = (uintptr_t)__builtin_va_arg(ap, void *);
            buf_puts(buf, size, &pos, "0x", -1);
            buf_u64(buf, size, &pos, (u64)v, 16, false, 16, '0');
            break;
        }
        case 's': {
            const char *s = __builtin_va_arg(ap, const char *);
            if (!s) s = "(null)";
            buf_puts(buf, size, &pos, s, prec);
            break;
        }
        case 'c': {
