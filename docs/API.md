...
- vmm_query_flags(space, virt) → leaf PTE flag bits (VMM_F_*), or 0 if the page is not present. The physical address bits are masked out.
...
- **`vmm_set_flags(space, virt, count, flags)`**: Update PTE protection flags for a page range. Used by sys_mprotect.
...
- **`hpet_init()`**: Locate the HPET via ACPI, map its MMIO, start the counter. Safe to call once after acpi_init().
- **`hpet_available()`**: true once hpet_init() has successfully started the timer.
- **`hpet_now_ns()`**: Nanoseconds since hpet_init() (monotonic, ~ns resolution). Returns 0 if the HPET is not available.
...
- **`krandom_init()`**: Seed the CSPRNG. Safe to call once, early in boot. krandom_bytes() will lazily self-seed if called first.
- **`krandom_bytes(buf, n)`**: Fill buf with n cryptographically-strong bytes.
- **`krandom_u64()`**: Return a random 64-bit value.
- **`krandom_add_entropy(buf, n)`**: Mix caller-supplied entropy into the pool (e.g. from /dev/random writes or interrupt timing jitter).
...
- **`elf_load_exec(proc, path, argv, envp, out_entry, out_space, out_rsp)`**: Load an ELF binary or shebang script from VFS into process memory space with System V AMD64 stack.
...
## `userland/build/home/azami/libc/include/az/ipc.h`
...
## `userland/build/usr/include/az/ipc.h`
...
## `userland/build/usr/include/sys/cdefs.h`

- **`/y #define	__STRING(x)	"x" #if !defined(__CC_SUPPORTS___INLINE) #define	__const				/* delete pseudo`**: ANSI C keywords
- /alias\",11,0,0,0");	\ __asm__(".stabs \"_/*

## `userland/build/usr/local/include/az/ipc.h`
...
## `userland/build/usr/src/azami-libc/include/az/ipc.h`
...
## `userland/build/usr/src/libc/include/az/ipc.h`
